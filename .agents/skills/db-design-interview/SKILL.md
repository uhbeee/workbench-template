...
After completing all interview phases, produce the deliverables as a Markdown-first design artifact per `context/standards/html-plan-standard.md`. Save as `db-design.md` with the ERD as a Mermaid code block, access-pattern table, decisions, risks, and exportable SQL/DDL notes — Mermaid renders inline in most editors and on GitHub, so MD already carries the diagram without a separate HTML page. Generate a `db-design.html` companion only when the design has substantial visual content beyond the Mermaid ERD (e.g., schema migration timeline SVG, side-by-side comparison of competing designs).

Generate the content in this order:
