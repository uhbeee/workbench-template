...
# Worktree skill script links: each worktree skill needs a skill-local scripts/
# entry, while all code stays in scripts/worktrees.
header "Skill Script Links"
ensure_worktree_skill_script_links || warn "Some worktree skill script links could not be created"
...
if is_windows; then
  ...
  worktree_root=$(parse_yaml_value "worktrees.root" 2>/dev/null || echo "C:/worktrees")
...
if command -v codex &>/dev/null; then
  ...
  pass "codex found: $(command -v codex) ($codex_version)"
else
  info "codex not found — attempting install"
  if is_mac && command -v brew &>/dev/null; then
    if brew install --cask codex; then
      pass "Installed codex via Homebrew"
    else
      fail "brew install --cask codex failed — install manually"
    fi
  elif command -v npm &>/dev/null; then
    if npm install -g @openai/codex; then
      pass "Installed codex via npm"
    else
      fail "npm install -g @openai/codex failed — install manually"
    fi
  else
    warn "Neither brew (mac) nor npm found — install codex manually"
  fi
fi

# ─── Step 4c: Claude and Codex aliases ────────────────────────────────────
header "Claude & Codex Aliases"
...
# Ensure alias file exists so grep/append always work
touch "$alias_file"

if grep -qF "alias cc=" "$alias_file"; then
  pass "Claude aliases (cc, ccd) in $(basename "$alias_file") already configured"
else
...
if grep -qE "^alias co=" "$alias_file"; then
  pass "Codex aliases (co, coy) in $(basename "$alias_file") already configured"
else
  cat >> "$alias_file" << 'CODEX_ALIASES'

# Codex CLI shortcuts (added by workbench setup)
alias co='codex'
alias coy='codex --yolo'
CODEX_ALIASES
  pass "Added co and coy aliases to $(basename "$alias_file")"
fi
...
if is_windows; then
  ...
  if [[ -n "$ps_profile" ]]; then
    ...
    touch "$ps_profile_unix"

    if grep -qF "Set-Alias cc claude" "$ps_profile_unix"; then
      pass "Claude aliases (cc, ccd) in PowerShell profile already configured"
    else
...
PS_ALIASES
    ...
    if grep -qF "Set-Alias co codex" "$ps_profile_unix"; then
      pass "Codex aliases (co, coy) in PowerShell profile already configured"
    else
      cat >> "$ps_profile_unix" << 'PS_CODEX_ALIASES'

# Codex CLI shortcuts (added by workbench setup)
Set-Alias co codex
function coy { codex --yolo @args }
PS_CODEX_ALIASES
      pass "Added co and coy aliases to PowerShell profile"
    fi
  else
  ...
  for i in "${!SKILL_TARGET_DIRS[@]}"; do
    ...
    for skill in "${GLOBAL_SKILLS[@]}"; do
      ...
      if create_dir_link "$src" "$dst"; then
        ...
        warn "$skill — could not create link at $skills_dst"
      fi
    done

    pass "$label global skills: $created linked, $skipped already set up"
    total_created=$((total_created + created))
    total_skipped=$((total_skipped + skipped))
  done

  pass "Global skills total: $total_created linked, $total_skipped already set up"
fi

# ─── Step 5b: Workbench root pointer ─────────────────────────────────
header "Workbench Root Pointer"

for wb_root_file in "$HOME/.claude/workbench-root" "$HOME/.codex/workbench-root"; do
  mkdir -p "$(dirname "$wb_root_file")"
  echo "$REPO_ROOT" > "$wb_root_file"
  pass "Wrote workbench root to $wb_root_file"
done
...
# ─── Step 7b: Worktree-isolation edit guard (Claude + Codex) ──────────────
header "Worktree-Isolation Guard"

if [[ -f "$REPO_ROOT/scripts/setup-edit-guard.sh" ]]; then
  REPO_ROOT="$REPO_ROOT" bash "$REPO_ROOT/scripts/setup-edit-guard.sh" || warn "Edit-guard install reported an issue (see above)"
else
  info "setup-edit-guard.sh not found — skipping edit-guard install"
fi
